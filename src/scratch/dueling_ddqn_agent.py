...
from src.scratch.models import DuelingDQN

class DuelingDDQNAgent:
    """
    Dueling Double Deep Q-Network (Dueling DDQN) Agent.
    """
    def __init__(
        self,
        state_shape: tuple,
        ...
        """
        Initialize the DuelingDDQNAgent.
        ...
        # Setup networks using DuelingDQN
        self.online_net = DuelingDQN(state_shape, num_actions).to(device)
        self.target_net = DuelingDQN(state_shape, num_actions).to(device)
    ...
    def update(self) -> float:
        ...
        # Compute targets for next states using Double DQN formula
        with torch.no_grad():
            # Double DQN: Choose action with online_net, evaluate action with target_net
